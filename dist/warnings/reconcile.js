...
export function reconcileErrors(events, turns) {
    const closed = new Set(); // tool_use_ids that already have a post from the hook
    const preById = new Map();
    for (const e of events) {
        if (!e.toolUseId)
            continue;
        if (e.phase === "post")
            closed.add(e.toolUseId);
        else
            preById.set(e.toolUseId, e);
    }
    const synthetic = [];
    for (const turn of turns) {
        if (!turn.isError || !turn.toolUseId)
            continue;
        if (closed.has(turn.toolUseId))
            continue;
