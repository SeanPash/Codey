...
describe("detectLoop", () => {
    it("warns when the same tool+inputHash repeats >= threshold consecutively", () => {
        const events = Array.from({ length: 6 }, (_, i) => ev({ id: String(i), phase: "pre", tool: "T", inputHash: "same", timestamp: i }));
    ...
    it("does not warn below threshold", () => {
        const events = Array.from({ length: 3 }, (_, i) => ev({ id: String(i), phase: "pre", inputHash: "same", timestamp: i }));
...
describe("detectRepeatError", () => {
    it("warns when the same tool errors with the same text >= threshold", () => {
        const events = Array.from({ length: 3 }, (_, i) => ev({ phase: "post", tool: "T", isError: true, errorText: "boom", timestamp: i }));
