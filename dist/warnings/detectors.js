...
export function detectLoop(events, threshold) {
    ...
    for (let i = pres.length - 1; i >= 0; i--) {
        if (pres[i].tool + "|" + pres[i].inputHash === key)
            count++;
        else
            break;
    }
...
export function detectRepeatError(events, threshold) {
    ...
    for (let i = errs.length - 1; i >= 0; i--) {
        if (errs[i].tool + "|" + (errs[i].errorText ?? "") === key)
            count++;
        else
            break;
    }
...
export function detectHang(openCalls, now, thresholdMs) {
