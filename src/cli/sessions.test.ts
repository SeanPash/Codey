...
import { addSessionToGroup, createGroup } from "../store/session-group-store.js";
...
describe("listSessions", () => {
  ...
  it("includes group tags separately from the saved flag", () => {
    const now = Date.now();
    const grouped = join(dir, "grouped-one");
    mkdirSync(grouped);
    writeFileSync(join(grouped, "events.jsonl"), '{"phase":"pre"}\n');
    writeSaved(grouped, true);
    const group = createGroup(dir, "Launch work")!;
    addSessionToGroup(dir, group.id, "grouped-one");

    const item = listSessions(dir, now).find((s) => s.id === "grouped-one")!;
    expect(item.saved).toBe(true);
    expect(item.groups).toEqual([{ id: group.id, name: "Launch work" }]);
  });
