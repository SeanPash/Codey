...
export function buildNarrationPrompt(events: ToolEvent[], mode: Mode): string {
  ...
  const instruction =
    mode === "teach"
      ...
      : mode === "deep"
      ? "Write exactly two sentences, each ending in a period and each about 18 words or fewer. First sentence: what Claude is doing and why, naming the real file and the actual function, value, or behavior being changed and what it now does differently. Second sentence: how that works or what it connects to. Do not merge them into one long sentence with \"while\" or \"and\". State the mechanism itself, not filler like \"implements new logic\" or \"updates the system\". For example: \"Claude is editing helper.ts so the loss function drops the opponent's defense by one. That is the rule that lets a winner press their edge after an exchange.\""
