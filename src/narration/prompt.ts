...
export function buildNarrationPrompt(events: ToolEvent[], mode: Mode): string {
  ...
  const instruction =
    mode === "teach"
      ? "Write exactly two sentences for someone learning to code, about 40 words total. Sentence one: the specific change or check, naming the real file and the function, value, or rule it touches and what that makes the code do differently. Sentence two: teach the one concept behind it and define any term you use. Lead with the work itself, not a throat-clearing intro, and never write a third sentence."
      : mode === "deep"
      ? "Write exactly two sentences, each ending in a period and each about 18 words or fewer. First sentence: what Claude is doing and why, naming the real file and the actual function, value, or behavior involved. Second sentence: when this is a change, how it works or what it connects to; when this is an investigation, the hypothesis it is testing and what the result would confirm or rule out. Do not merge them into one long sentence with \"while\" or \"and\". State the mechanism itself, not filler like \"implements new logic\" or \"updates the system\". For example: \"Claude is editing helper.ts so the loss function drops the opponent's defense by one. That is the rule that lets a winner press their edge after an exchange.\" Or: \"Claude is comparing the served timeline page with the source file to see why the token labels are missing. A match in source would point at a stale browser tab, not a markup bug.\""
      : "Write one sentence, no more than about 20 words, saying what Claude is doing right now, naming the real file and the specific function or command involved, and what it is checking or changing. Be concrete, not generic.";
